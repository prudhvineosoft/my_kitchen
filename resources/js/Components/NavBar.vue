<script>
import { Link } from '@inertiajs/vue3';
import { ref, watch } from 'vue';
export default {
    setup() {

        const isNavHidden = ref(true);

        function toggleNav() {
            isNavHidden.value = !isNavHidden.value;
        }

        watch(
            isNavHidden,
            (newValue, oldValue) => {
                const navContent = document.getElementById('nav-content');
                if (navContent) {
                    navContent.classList.toggle('hidden', newValue);
                }
            }
        );

        return {
            isNavHidden,
            toggleNav
        };
    }
}
</script>
<template>
    <div class="bg-gray-400 font-sans leading-normal tracking-normal">

        <nav class="flex items-center justify-between flex-wrap bg-gray-800 p-3 fixed w-full z-10 top-0">
            <div class="flex items-center flex-shrink-0 text-white mr-6">
                <a class="text-white no-underline hover:text-white hover:no-underline" href="#">
                    <span class="text-2xl pl-2"><i class="em em-grinning"></i> Brand McBrandface</span>
                </a>
            </div>

            <div class="block lg:hidden">
                <button @click="toggleNav"
                    class="flex items-center px-3 py-2 border rounded text-gray-500 border-gray-600 hover:text-white hover:border-white">
                    <svg class="fill-current h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <title>Menu</title>
                        <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" />
                    </svg>
                </button>
            </div>

            <div class="w-full flex-grow lg:flex lg:items-center lg:w-auto hidden lg:block pt-6 lg:pt-0" id="nav-content">
                <Link :href="route('login')"
                    class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">
                Log in</Link>
                <ul class="list-reset lg:flex justify-end flex-1 items-center">
                    <li class="mr-3" @click="toggleNav">
                        <RouterLink class="inline-block  px-4 text-white no-underline hover:text-blue-400" to="/">Home
                        </RouterLink>
                    </li>
                    <li class="mr-3" @click="toggleNav">
                        <RouterLink class="inline-block  px-4 text-white no-underline hover:text-blue-400" to="/about">
                            About
                        </RouterLink>
                    </li>
                    <li class="mr-3">
                        <a class="inline-block text-white no-underline hover:text-blue-400  px-4" href="#">link</a>
                    </li>

                    <li class="mr-3">
                        <a class="inline-block text-white no-underline hover:text-blue-400  px-4" href="#">link</a>
                    </li>
                </ul>

            </div>
        </nav>
    </div>
</template>



<style  scoped>
nav a.router-link-exact-active {
    color: blue;
}
</style>
